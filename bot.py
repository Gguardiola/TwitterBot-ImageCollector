...
hdlr = logging.FileHandler('twitterbot.log')
...
def picRandomizer():
  print("HORA JOSEO")
  toss = random.randint(
      0, 20)  #decisión random de si lee los favs o la carpeta de joseos
  ...
  if toss >= 0 and toss <= 9:
      ...
      try:
          flog = open(file=favlog, mode="r+",
                      encoding="utf-8")  ##Abre el log en read write
          ...
          if len(lines) != len(flines):
              while i < len(flines):  ##recorre las lineas buscando el joseo
                  if flines[i] == (tuit):
                      ...
                      tuit = lines[random.randint(0, len(lines)) -
                                   1]  #selecciona una linea random
                      i = 0
                  else:
                      i = i + 1
          else:
              ...
              api.update_status(
                  status=
                  "Si ves este tuit es porque me he quedado sin memes para subir y el bot ha entrado en pánico y se ha suicidado"
              )
              sys.exit()
      ...
      if "twimg.com" in tuit:  #si tiene imagen:
          ...
          reference = reference[len(reference) -
                                1]  #saca el nombre del archivo local
          reference = reference.split("\n")
          ...
          try:
            ...
            tuitear = api.update_status(
                status=stat,
                media_ids=[subir.media_id
                           ])  ##devuelve la id de la foto y la tuitea
          except:
            picRandomizer()
      else:
  ...
  ##============================================================================================================
      ...
      vector_joseos = glob.glob(folder +
                                "*")  ##Genera la ruta de todos los ficheros
      media = vector_joseos[random.randint(0, len(vector_joseos)) -
                            1]  #selecciona un fichero random
      try:
          ...
          if len(vector_joseos) != len(lines):
              while i < len(lines):  ##recorre las lineas buscando el joseo
                  if lines[i] == (media + "\n"):
                      ...
                      media = vector_joseos[random.randint(
                          0, len(vector_joseos)) -
                                            1]  #selecciona un fichero random
                      i = 0
                  else:
      ...
      josear = josear[1].split(
          "_"
      )  #splitea el filename y nos queda el formato: [0] = CAP [1] = status [2..] = garbage
      ...
      if josear[0].lower() == "cap":
          ...
          tuitear = api.update_status(
              status=josear[1])  ##devuelve la id de la foto y la tuitea
      else:
          print("JOSEO SIN CAPTION")
          subir = api.media_upload(media)  #sube la foto a twitter
          tuitear = api.update_status(status="", media_ids=[
              subir.media_id
          ])  ##devuelve la id de la foto y la tuitea
...
#            api.get_favorites).items(max):  ##selecciona los X ultimos favs
#
#        stat = api.get_status(favorite.id, tweet_mode="extended")
#        ##EN CASO DE TENER VIDEO: guarda el url del video
#        vidcheck = False
#        try:
#            vidurl = stat.extended_entities["media"][0]["video_info"][
#                "variants"][1]["url"]
...
#            pass
#
#        stat = str(stat.full_text)
#        stat = stat.split("https://t.co/")
#        stat = stat[0]  ##elimina el enlace de imagen malo
#
...
#            try:
#                wget.download(media,
#                              joseo_favs_dir)  ##guardamos imagenes en imgtxt
#            except FileNotFoundError:
#                try:
#                    os.mkdir("imgtxt")
...
#            tuit = str(
#                stat
#            ) + "_PIC_" + media  ##saca la imagen real y la concatena al tuit
#        else:
#            tuit = str(stat)
#        #print(tuit)
#        f.write(tuit + "\n")
...
try:
  ...
  ##HORARIOS
  schedule.every().day.at("09:30").do(picRandomizer)  #10:30
  print("Programado mañana (10:30)")
  schedule.every().day.at("15:00").do(picRandomizer)  #16:00
  print("Programado mediodia (16:00)")
  schedule.every().day.at("17:35").do(picRandomizer)  #18:30
  print("Programado Tarde-noche (18:35)")
  while True:
